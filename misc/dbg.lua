--- Lua调试器
-- 参考 https://github.com/pkulchenko/MobDebug
...
dbg.line = serpent.line
dbg.dump = serpent.dump
dbg.linemap = nil
dbg.load = load
...
local function in_debugger()
  ...
  -- only need to check few frames as dbg frames should be close
  for i = 3, 7 do
    local info = debug.getinfo(i, "S")
...
local function is_pending(peer)
  ...
  if not buf and checkcount >= dbg.checkcount then
...
local function debug_hook(event, line)
  ...
  elseif event == "line" then
    if dbg.linemap then
      local ok, mappedline = pcall(dbg.linemap, line, debug.getinfo(2, "S").source)
    ...
    checkcount = dbg.checkcount -- force check on the next command
    ...
    if (lastsource ~= caller.source) then
      ...
      -- for example when they call load('...', 'filename.lua').
    ...
    if status and res and res ~= 'stack' then
      if not abort and res == "exit" then dbg.onexit(1, true); return end
      if not abort and res == "done" then dbg.done(); return end
      abort = res
...
local function stringify_results(params, status, ...)
  ...
  for i = 1, select('#', ...) do -- stringify each of the returned values
    local ok, res = pcall(dbg.line, select(i, ...), params)
  ...
  return pcall(dbg.dump, t, {sparse = false})
end

local function isrunning()
  return coro_debugger and (coroutine.status(coro_debugger) == 'suspended' or coroutine.status(coro_debugger) == 'running')
end
...
local function debugger_loop(sev, svars, sfile, sline)
  ...
  while true do
    local line, err
    if dbg.yield and server.settimeout then server:settimeout(dbg.yieldtimeout) end
    while true do
      line, err = server:receive("*l")
      if not line then
        if err == "timeout" then
          if dbg.yield then dbg.yield() end
    ...
    elseif command == "EXEC" then
      ...
      if chunk then
        ...
        if status then
          if dbg.onscratch then dbg.onscratch(res) end
          server:send("200 OK " .. tostring(#res) .. "\n")
          server:send(res)
        else
          -- fix error if not set (for example, when load is not present)
      ...
      if abort == nil then -- no LOAD/RELOAD allowed inside start()
        ...
        if size == 0 and name == '-' then -- RELOAD the current script being debugged
          ...
          if chunk then -- LOAD a new script for debugging
            local func, res = dbg.load(chunk, "@"..name)
            if func then
              server:send("200 OK 0\n")
              debugee = func
              coroutine.yield("load")
    ...
    elseif command == "SETW" then
      ...
      if exp then
        local func, res = dbg.load("return(" .. exp .. ")")
        if func then
    ...
    elseif command == "STACK" then
      ...
      if ev and ev ~= events.STACK then
        ...
        local ok, res = pcall(dbg.dump, vars, params)
        if ok then
          server:send("200 OK " .. tostring(res) .. "\n")
        else
      ...
      if stream and mode and stream == "stdout" then
        ...
        genv.print = default and iobase.print or coroutine.wrap(function()
          ...
          while true do
            ...
            for n = 1, #tbl do
              tbl[n] = select(2, pcall(dbg.line, tbl[n], {nocode = true, comment = false})) end
...
local function connect(controller_host, controller_port)
  ...
  if sock.settimeout then sock:settimeout(dbg.connecttimeout) end
...
local function start(controller_host, controller_port)
  ...
  controller_port = lastport or dbg.port

  local err
  server, err = dbg.connect(controller_host, controller_port)
  if server then
...
local function controller(controller_host, controller_port, scratchpad)
  ...
  if server then
    ...
    while true do
      ...
      if scratchpad then checkcount = dbg.checkcount end -- force suspend right away

      coro_debugee = coroutine.create(debugee)
      debug.sethook(coro_debugee, debug_hook, HOOKMASK)
      local status, err = coroutine.resume(coro_debugee, unpack(arg or {}))
      ...
      if abort then
        ...
        if status then -- no errors
          if coroutine.status(coro_debugee) == "suspended" then
        ...
        elseif err and not string.find(tostring(err), deferror) then
          ...
          local status, err = coroutine.resume(coro_debugger, events.RESTART, capture_vars(0))
...
local function handle(params, client, options)
  ...
  elseif command == "eval" or command == "exec"
      or command == "load" or command == "load"
  ...
  elseif command == "stack" then
    ...
    if status == "200" then
      ...
      for _,frame in ipairs(stack) do
        print(dbg.line(frame[1], {comment = false}))
      end
      return stack
    elseif status == "401" then
...
dbg.setbreakpoint = set_breakpoint
dbg.removebreakpoint = remove_breakpoint
dbg.listen = listen
dbg.loop = loop
dbg.scratchpad = scratchpad
dbg.handle = handle
dbg.connect = connect
dbg.start = start
dbg.on = on
...
dbg.coro = coro
dbg.done = done
dbg.pause = function() step_into = true end
dbg.yield = nil -- callback
dbg.output = output
dbg.onexit = os and os.exit or done
dbg.onscratch = nil -- callback
dbg.basedir = function(b) if b then basedir = b end return basedir end
