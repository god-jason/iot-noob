...
local function fota_task(cbFnc, opts)
    ...
    elseif code == -5 then
        ...
        if string.find(url, hziot) then
            ...
            elseif code == 17 then
                log.info("无权限",
                    "设备会上报imei、固件名、项目key,服务器会以此查出设备、固件、项目三 条记录，如果 这三者不在同一个用户名下，" ..
                        "就会认为无权限。设备不在项目key对应的账户下，可寻找合宙技术支持查询该设备在哪个账户下，核实情况后可修改设备归属")
            elseif code == 21 then
            ...
            elseif code == 26 then
                log.info("无效的固件",
                    "固件名称错误,项目中没有对应的固件,也有可能是用户自己修改了固件名称,可对照升级日志中设备当前固件名与升级配置中固件名是否相同" ..
                        "(固件名称,固件功能要完全一致,只是版本号不同)")
            elseif code == 27 then
                log.info("已是最新版本",
                    "1.设备的固件/脚本版本高于或等于云平台上的版本号 2.用户项目升级配置中未添加该设备 3.云平台升级配置中，是否升级配置为否")
            elseif code == 40 then
                log.info("循环升级",
                    "云平台进入设备列表搜索被禁止的imei,解除禁止升级即可. 云平台防止模块在升级失败后,反复请求升级导致流量卡流量耗尽," ..
